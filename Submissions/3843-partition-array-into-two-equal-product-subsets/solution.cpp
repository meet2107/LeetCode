...
public:
    ...
    bool help(ll idx, ll p, vector<int> &nums, ll target, vll &vis) {
        if (p > target){
            return false;
        }
        ...
        string cur = to_string(idx) + "*" + to_string(p);
        if(dp.count(cur)){
            return dp[cur];
        }

        bool ntk = help(idx + 1, p, nums, target, vis);
        vis[idx] = true;
        bool tk = help(idx + 1, p * nums[idx], nums, target, vis);
        vis[idx] = false;

        return dp[cur] = ntk || tk;
    }

    bool checkEqualPartitions(vector<int>& nums, ll target) {
        ll n = nums.size();
        vll vis(n, 0);
        return help(0, 1, nums, target, vis);
    }
};
